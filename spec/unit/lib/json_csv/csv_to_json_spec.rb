...
describe JsonCsv::CsvToJson do
  ...
  describe '.csv_file_to_hierarchical_json_hash' do
  ...
  describe '.csv_row_hash_to_hierarchical_json_hash' do
    ...
    context 'works as expected, trimming string values and removing empty values by default' do
      ...
      it do
        expect(dummy_class.csv_row_hash_to_hierarchical_json_hash(csv_row_hash,
                                                                  field_casting_rules)).to eq(expected_cleaned)
      end
    end
    ...
    context 'does not strip string values when strip_value_whitespace arg is false' do
      ...
      it do
        expect(
          dummy_class.csv_row_hash_to_hierarchical_json_hash(csv_row_hash, field_casting_rules, strip_value_whitespace)
        ).to eq(expected_without_whitespace_strip)
      end
    end
  end

  describe '.put_value_at_json_path' do
    let(:hsh) do
      {
    ...
    it 'creates new intermediate structures when creating a new nested value, '\
      'and can handle spaces and other non-alpha chars in the nested hash keys' do
      dummy_class.put_value_at_json_path(hsh, 'new top level key[1].deeper_value[3].low-level cool value!',
                                         'deeply nested value')
    ...
    it 'can handle json paths that have consecutive string hash indexes' do
      dummy_class.put_value_at_json_path(hsh, 'new top level key.deeper_value.low-level cool value!',
                                         'deeply nested value')
  ...
  context 'json_path conversion' do
    ...
    describe '.pieces_to_json_path' do
    ...
    describe '.json_path_to_pieces' do
      it 'generates the expected pieces' do
  ...
  describe '.apply_field_casting_type' do
  ...
  describe '.real_json_path_to_field_casting_rule_pattern' do
    it 'converts as expected' do
      expect(
        dummy_class.real_json_path_to_field_casting_rule_pattern('related_books[1].notes_from_reviewers[0]')
      ).to eq('related_books[x].notes_from_reviewers[x]')
    end
  end
end
